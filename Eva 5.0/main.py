...
def Wake_Word_Engine_Thread_Management():
    ...
    try:
        ...
        # INITIATE PYAUDIO OBJECT, LISTEN TO THE DEFAULT MIC ON 1 CHANNEL, WITH A RATE OF 16000 HZ AND A BUFFER OF 1000 FRAMES
        mic = pyaudio.PyAudio()
        stream = mic.open(format=pyaudio.paInt16, channels=1, rate=16000, input=True, frames_per_buffer=1000)
        stream.start_stream()

        while True:
            # READ FROM THE AUDIO DATA STREAM A 1000 FRAMES PER CYCLE
